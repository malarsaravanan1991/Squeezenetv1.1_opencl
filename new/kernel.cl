			...
			for (int k = 0; k < 3; k++)//row
			{
				__attribute__((xcl_pipeline_loop))
					for (int l = 0; l < 3; l++)//col
					{
						...
						int value = input_im[(i * 2 + k) * input_size + j * 2 + l];
						if (value > tmp)
							tmp = value;
...
__kernel void conv2(global const int* input_image_conv1x1, global const int* filter_conv1x1, global int* output_conv1x1_layer,
